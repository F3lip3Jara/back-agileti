...
CREATE OR REPLACE VIEW proveedores( id,rut,nombre,nombre_fantasia,giro,pais , pais_id ,region, region_id,comuna, comuna_id,ciudad,ciudad_id, direccion,numero,telefono ,es_cliente, es_proveedor,mail , activado , place , lat , lng, created_at, updated_at,empId)AS ( SELECT prvId ,prvRut, prvNom, prvNom2, prvGiro,b.paiDes , b.`paiId` , c.regDes , c.`regId`,d.comDes, d.`comId` ,e.ciuDes , e.`ciuId`, prvDir, prvNum , prvTel, prvCli, prvPrv, prvMail , prvAct, prvPlace, prvLat, prvLong , a.created_at , a.updated_at  , a.`empId` from parm_proveedor a join parm_pais b on a.paiId = b.paiId join parm_region c on a.regId = c.regId join parm_comuna d on a.comId = d.comId join parm_ciudad e on a.ciuId = e.ciuId );
